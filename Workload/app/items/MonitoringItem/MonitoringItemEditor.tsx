...
import { MonitoringItemEmptyView } from "./MonitoringItemEmptyView";
import { MonitoringItemDefaultView } from "./MonitoringItemDefaultView";
import { MonitoringItemRibbon } from "./MonitoringItemRibbon";
import "./MonitoringItem.scss";

/**
 * Different views that are available for the Monitoring item
 */
...
export function MonitoringItemEditor(props: PageProps) {
  ...
  const [item, setItem] = useState<ItemWithDefinition<MonitoringItemDefinition>>();
  const [saveStatus, setSaveStatus] = useState<SaveStatus>(SaveStatus.NotSaved);
  const [currentDefinition, setCurrentDefinition] = useState<MonitoringItemDefinition>({});
  ...
  async function loadDataFromUrl(pageContext: ContextProps, pathname: string): Promise<void> {
    ...
    setIsLoading(true);
    var LoadedItem: ItemWithDefinition<MonitoringItemDefinition> = undefined;
    if (pageContext.itemObjectId) {
      // for Edit scenario we get the itemObjectId and then load the item via the workloadClient SDK
      try {
        LoadedItem = await getWorkloadItem<MonitoringItemDefinition>(
          workloadClient,
  ...
  async function saveItem() {
    ...
    try {
      successResult = await saveWorkloadItem<MonitoringItemDefinition>(
        workloadClient,
  ...
  const EmptyViewWrapper = () => {
    ...
    return (
      <MonitoringItemEmptyView
        workloadClient={workloadClient}
        item={item}
        onNavigateToGettingStarted={() => {
          setCurrentDefinition(prev => ({ ...prev, message: "SOAK CONNECT Item!" }));
          setSaveStatus(SaveStatus.NotSaved);
  ...
  return (
    <ItemEditor
      isLoading={isLoading}
      loadingMessage={t("MonitoringItemEditor_Loading", "Loading item...")}
      ribbon={(context) => (
        <MonitoringItemRibbon
          {...props}
          viewContext={context}
